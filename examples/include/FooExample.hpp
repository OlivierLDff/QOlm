...
public:
    ...
    QString toString() const { return QStringLiteral("Foo: {foo=") + QString::number(getFoo()) + QStringLiteral("}"); }
Q_SIGNALS:
    void fooChanged();
};
...
class FooList : public qolm::QOlm<Foo>
{
    Q_OBJECT
public:
    FooList(QObject* parent = nullptr, const QList<QByteArray>& exposedRoles = {}, const QByteArray& displayRole = {}) :
        ...
        onInserted(
            [](Foo* foo, int index)
            {
                ...
                qInfo("callback : onInserted 0x%s : %d",
                    qPrintable(QString::number(reinterpret_cast<quintptr>(foo), 16)), index);
            });
        onRemoved(
            [](Foo* foo, int index)
            {
        ...
        onMoved(
            [](Foo* foo, int from, int to)
            {
                ...
                qInfo("callback : onMoved 0x%s : %d %d",
                    qPrintable(QString::number(reinterpret_cast<quintptr>(foo), 16)), from, to);
            });
        ...
        connect(this, &FooList::objectInserted, this,
            [](QObject* obj, int index)
            {
                qInfo("signal   : onInserted 0x%s : %d",
                    qPrintable(QString::number(reinterpret_cast<quintptr>(qobject_cast<Foo*>(obj)), 16)), index);
            });
        ...
        connect(this, &FooList::objectMoved, this,
            [](QObject* obj, int from, int to)
            {
                qInfo("signal   : onRemoved 0x%s : %d %d",
                    qPrintable(QString::number(reinterpret_cast<quintptr>(qobject_cast<Foo*>(obj)), 16)), from, to);
            });
    }
    ...
    QString toString() const
    {
        ...
        for(const auto it: *this) res += QStringLiteral("foo") + QString::number(it->getFoo()) + QStringLiteral(", ");
        res += QStringLiteral("}");
...
protected:
    void onObjectAboutToBeInserted(Foo* item, int row) override final
    {
        qInfo("override : onItemAboutToBeInserted 0x%s : %d",
            qPrintable(QString::number(reinterpret_cast<quintptr>(item), 16)), row);
    }
    void onObjectInserted(Foo* item, int row) override final
    {
        qInfo("override : onItemInserted 0x%s : %d", qPrintable(QString::number(reinterpret_cast<quintptr>(item), 16)),
            row);
    }
    void onObjectAboutToBeMoved(Foo* item, int src, int dest) override final
    {
        qInfo("override : onItemAboutToBeMoved 0x%s : %d %d",
            qPrintable(QString::number(reinterpret_cast<quintptr>(item), 16)), src, dest);
    }
    void onObjectMoved(Foo* item, int src, int dest) override final
    {
        qInfo("override : onItemMoved 0x%s : %d %d", qPrintable(QString::number(reinterpret_cast<quintptr>(item), 16)),
            src, dest);
    }
    ...
    {
        qInfo("override : onItemRemoved 0x%s : %d", qPrintable(QString::number(reinterpret_cast<quintptr>(item), 16)),
            row);
    }
};
