...
public:
    ...
    void insert(int idx, _Object* item)
    {
        if(item != nullptr)
        {
            objectAboutToBeInsertedNotify(item, idx);
            beginInsertRows(noParent(), idx, idx);
            _objects.insert(idx, item);
            referenceItem(item);
            updateCounter();
            endInsertRows();
            objectInsertedNotify(item, idx);
        }
    }
...
public:
    ...
    {
        explicit _BaseCallbackArgs(_Object* object)
            : object(object)
        {
        }
