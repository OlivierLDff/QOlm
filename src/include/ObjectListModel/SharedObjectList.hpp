#ifndef __OBJECTLISTMODEL_SHAREDOBJECTLIST_HPP__
#define __OBJECTLISTMODEL_SHAREDOBJECTLIST_HPP__
...
#define OBJLISTMODEL_SHAREDOBJECTLIST_FOREACH_PTR_IN_QLIST(_type_, _var_, _list_) \
...
OBJLISTMODEL_API_ class SharedObjectListBase : public QAbstractListModel // abstract Qt base class
{
    Q_OBJECT
...
public:
    explicit SharedObjectListBase(QObject * parent = Q_NULLPTR) : QAbstractListModel(parent) { }
...
class SharedObjectList : public SharedObjectListBase
{
public:
    explicit SharedObjectList(QObject *          parent      = Q_NULLPTR,
                                  const QList<QByteArray> & exposedRoles = QList<QByteArray>(),
                                  const QByteArray & displayRole = QByteArray(),
                                  const QByteArray & uidRole     = QByteArray())
        : SharedObjectListBase(parent)
        , _count(0)
        ...
        , _metaObj(ItemType::staticMetaObject)
    {
        ...
        for(int propertyIdx = 0, role =(baseRole() +1); propertyIdx < len; propertyIdx++, role++)
        {
            ...
            {
                static const QByteArray CLASS_NAME =(QByteArrayLiteral("SharedObjectList<") % _metaObj.className() % '>');
...
public: // C++ API
    ...
    void clear(void) Q_DECL_FINAL
    {
        if(!_items.isEmpty())
        {
            ...
            OBJLISTMODEL_SHAREDOBJECTLIST_FOREACH_PTR_IN_QLIST(ItemType, item, _items)
            {
    ...
    void append(const QList<QSharedPointer<ItemType>> & itemList)
    {
        if(!itemList.isEmpty())
        {
            ...
            OBJLISTMODEL_SHAREDOBJECTLIST_FOREACH_PTR_IN_QLIST(ItemType, item, itemList)
            {
                referenceItem(item);
            }
            updateCounter();
    ...
    void remove(int idx) Q_DECL_FINAL
    {
        if(idx >= 0 && idx < _items.size())
        {
            ...
            QSharedPointer<ItemType> item = _items.takeAt(idx);
            dereferenceItem(item);
            updateCounter();
            endRemoveRows();
    ...
    void referenceItem(QSharedPointer<ItemType> item)
    {
        if(item != Q_NULLPTR)
        {
            ...
            for(QHash<int, int>::const_iterator it = _signalIdxToRole.constBegin(); it != _signalIdxToRole.constEnd(); ++it)
                connect(item.get(), item->metaObject()->method(it.key()), this, _handler, Qt::UniqueConnection);
...
private:
    void itemAboutToBeInserted(QSharedPointer<ItemType> item, int row)
    { onItemAboutToBeInserted(item, row); _onItemAboutToBeInserted(item, row); }
    void itemInserted(QSharedPointer<ItemType> item, int row)
    { onItemInserted(item, row); _onItemInserted(item, row); }
    void itemAboutToBeMoved(QSharedPointer<ItemType> item, int src, int dest)
    { onItemAboutToBeMoved(item, src, dest); _onItemAboutToBeMoved(item, src, dest); }
    void itemMoved(QSharedPointer<ItemType> item, int src, int dest)
    { onItemMoved(item, src, dest); _onItemMoved(item, src, dest); }
    void itemAboutToBeRemoved(QSharedPointer<ItemType> item, int row)
    { onItemAboutToBeRemoved(item, row); _onItemAboutToBeRemoved(item, row); }
    void itemRemoved(QSharedPointer<ItemType> item, int row)
...
private:
    void _onItemAboutToBeInserted(QSharedPointer<ItemType> item, int row)
    { emit SharedObjectListBase::itemAboutToBeInserted(item, row); }
    void _onItemInserted(QSharedPointer<ItemType> item, int row)
    { emit SharedObjectListBase::itemInserted(item, row); }
    void _onItemAboutToBeMoved(QSharedPointer<ItemType> item, int src, int dest)
    { emit SharedObjectListBase::itemAboutToBeMoved(item, src, dest); }
    void _onItemMoved(QSharedPointer<ItemType> item, int src, int dest)
    { emit SharedObjectListBase::itemMoved(item, src, dest); }
    void _onItemAboutToBeRemoved(QSharedPointer<ItemType> item, int row)
    { emit SharedObjectListBase::itemAboutToBeRemoved(item, row); }
    void _onItemRemoved(QSharedPointer<ItemType> item, int row)
    { emit SharedObjectListBase::itemRemoved(item, row); }
...
private: // data members
    ...
    QList<QSharedPointer<ItemType>>          _items;
    QHash<QString, QSharedPointer<ItemType>> _indexByUid;
};
 ...
 * \internal
 */
class _Test_SharedObjList : public SharedObjectList<_Test_TestSharedObj>
{
    Q_OBJECT
...
#endif // __OBJECTLISTMODEL_SHAREDOBJECTLIST_HPP__
